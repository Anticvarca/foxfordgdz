...
# ============ ФИЛЬТР ШКОЛЬНЫХ ЗАДАНИЙ ============
SCHOOL_WORDS = re.compile(
    r"(алгебр|геометри|математик|уравнени|пример|задач|формул|выражени|"
    r"русск|литератур|сочинени|орфограф|пунктуац|запят|букв|слов|"
    r"истори|дат|век|эпох|войн|революц|правител|"
    r"географ|карт|стран|климат|материк|"
    r"биолог|клетк|орган|животн|растен|фотосинтез|"
    r"физик|скорост|сил[ауы]|энерг|масс[ауы]|давлени|"
    r"хими|реакц|веществ|элемент|молекул|"
    r"информатик|алгоритм|программ|код|"
    r"английск|перевед|translate|"
    r"тест|вариант|упражнени|задание|"
    r"гимнази|школ|класс|урок|дз|домашк|"
    r"огэ|егэ|впр|экзамен|контрольн|"
    r"choose|select|answer|complete|fill|match|"
    r"реши|решить|реша|найди|найти|определ|вычисли|перевед|"
    r"вставь|встав|выбери|выбер|ответь|"
    r"что такое|почему|назови|выпиши|объясни|"
    r"докажи|сравни|составь|запиши|подчеркни|раскрой|укажи|посчитай|обозначь|"
    r"напиши|приведи|опиши|построй|постро|соотнеси|распредел|"
    r"прочитай|прочти|исправь|дополни|заполни|начерти)",
    re.IGNORECASE,
)

NON_SCHOOL_WORDS = re.compile(
    r"(айфон|iphone|ipad|android|андроид|ios|windows|macos|"
    r"app store|google play|testflight|altstore|sideloadly|mdm|"
    r"установи[ть]? приложени|скачать приложени|обновить приложени|"
    r"как настрои[ть]?|как почини[ть]?|как включи[ть]?|как выключи[ть]?|"
    r"wi-fi|вайфай|роутер|модем|"
    r"рецепт|приготов|борщ|пицц|пирог|салат|"
    r"крипт|биткоин|bitcoin|ton|usdt|ethereum|кошел[её]к|"
    r"порн|секс|18\+|"
    r"ставк|казин|букмекер|"
    r"погода|гороскоп|"
    r"мем|анекдот|шутк|"
    r"тикток|tiktok|instagram|инстаграм|youtube|ютуб|"
    r"roblox|роблокс|minecraft|майнкрафт|csgo|dota|"
    r"как заработа[ть]?|как стать|как похудет|"
    r"познакоми[ть]?ся|отношени|девушк|парн[яе]|любов)",
    re.IGNORECASE,
)
...
async def handle_text(message: types.Message):
    ...
    if not looks_like_task(message.text):
        log.info("handle_text: не школьное задание: %r", message.text[:60])
        save_last_message(user_id, "text", text=message.text)
        await message.answer(
            "🤔 Это не похоже на школьное задание.\n\n"
            "Я помогаю только с домашней работой по школьным предметам:\n"
            "математика, русский, литература, история, география, "
            "биология, физика, химия, информатика, английский.\n\n"
            "Примеры запросов:\n"
            "• «Реши 2x + 5 = 11»\n"
